...
export class InnService {
  static getRoomCost(roomType: RoomType): number {
    return TownConfigLoader.getInnRoomCost(roomType);
  }

  static getRoomHealRate(roomType: RoomType): number {
    return TownConfigLoader.getInnRoomHealRate(roomType);
  }
  ...
  static restoreSpellPoints(character: Character): Character {
    ...
    return {
      ...character,
      spellPoints: {
        mage: character.spellPoints.mage ? restorePool(character.spellPoints.mage) : undefined,
   ...
   */
  static calculatePartyHealPlan(characters: Character[], partyGold: number): PartyHealPlan {
    // Filter to living characters who need healing
    const livingChars = characters.filter((c) => c.status === CharacterStatus.OK && c.hp < c.maxHp);
  ...
  static executePartyRest(
    ...
    for (const charId of partyMembers) {
      ...
      const hpGained = Math.min(plan.weeksNeeded * healRate, char.maxHp - char.hp);
      ...
      while (updatedChar.hp === updatedChar.maxHp && LevelUpService.canLevelUp(updatedChar)) {
        ...
        if (levelUpResult.levelUpData.diedFromVitalityLoss) {
          break;
        }
      }
